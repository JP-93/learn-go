...
func main() {
	...
	doc, error := goquery.NewDocumentFromReader(resp.Body) // faz a query no html e faz o parse, gerar o documento usando o goquery
	check(error)
	title := doc.Find("title").Text() // aqui o parse do titulo, e retorno o html
	fmt.Println(title)
